...
MATLAB has the option to switch over, numerical values using ```==```, character vectors (strings) using ```strcmp()```, objects and cell arrays. The later two we will cover later on. Below see a simple numeric example that will see if the user wins a price or not:
